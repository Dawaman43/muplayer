use gtk4::prelude::*;
use gtk4::{ApplicationWindow, Button, Label, Box, Scale, Orientation, Frame, ListBox, TreeView, TreeViewColumn, TreeStore, CellRendererText, ScrolledWindow, Grid, Separator, SelectionModel, SingleSelection};
...
fn build_ui(app: &AdwApplication, player_state: &Arc<Mutex<PlayerState>>) {
    ...
    // Create tab content
    let all_songs_content = create_all_songs_list(&songs);
    let albums_content = create_albums_grid(&songs);
    let artists_content = create_artists_list(&songs);
    let folders_content = create_folders_tree(&songs);
...
fn scan_music_library() -> Vec<Song> {
    ...
    let search_paths = vec![
        "/home/dave/Music",
        "/home/dave/Downloads",           // ALL of Downloads, not just Downloads/Music
...
fn create_albums_grid(songs: &[Song]) -> gtk4::Widget {
    ...
    let mut albums: Vec<(String, String, String, String)> = Vec::new();
    ...
    for song in songs {
        ...
        if !seen.contains(&key) {
            ...
            albums.push((song.album.clone(), song.artist.clone(), "🎸".to_string(), song.title.clone()));
        }
    }

    for (i, (album, artist, icon, sample_title)) in albums.iter().enumerate().take(20) {
        let card = create_album_card(album, artist, icon, sample_title);
        let row = (i / 4) as i32;
        let col = (i % 4) as i32;
...
fn create_folders_tree(songs: &[Song]) -> gtk4::Widget {
    ...
    for song in songs {
        if let Some(parent) = song.path.parent() {
            ...
            // Group by main parent directory
